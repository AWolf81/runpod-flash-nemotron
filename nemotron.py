...
def make_seed_runner(hf_token: str):
    ...
    async def seed_model(_: dict | None = None) -> dict:
        import os
        ...
        if os.path.exists(model_path):
            return {"status": "already_present", "model_path": model_path}
...
async def seed_model_once() -> dict:
    hf_token = os.environ.get("HF_TOKEN")
    if not hf_token:
        raise SystemExit(
            "HF_TOKEN is required. Example: HF_TOKEN=hf_... python nemotron.py seed"
        )

    print(f"Running one-time remote seed job for volume '{VOLUME_NAME}'")
    print(f"Target path: {MODEL_PATH}")
    print("This downloads the model on a remote RunPod worker into /runpod-volume.")
    print("It is not a direct storage API import and may take a while...")

    seed_runner = make_seed_runner(hf_token)
    result = await seed_runner({})

    print()
    print(f"Seed result: {result['status']}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Usage:")
    print("  HF_TOKEN=hf_... python nemotron.py seed")
    print("  flash deploy")
    return 1
